...
public class ProductController {
	...
	public ResponseEntity<ResponseDto<ProductResponse>> createProduct(
		...
		return ResponseEntity
			...
			.body(ResponseDto.of(HttpStatus.CREATED, "제품 등록 성공", dto));
	}

	@Operation(summary = "회사별 제품 목록 조회", tags = "제품")
	@GetMapping("/by-company/{companyId}")
	public ResponseEntity<ResponseDto<List<ProductResponse>>> getProductsByCompany(
		@PathVariable Long companyId,
		@CurrentUser User currentUser
	) {
		List<ProductResponse> list = productService.listByCompany(companyId, currentUser);
		return ResponseEntity
			.ok(ResponseDto.of(HttpStatus.OK, "회사별 제품 목록 조회 성공", list));
	}
	...
	public ResponseEntity<ResponseDto<ProductResponse>> updateProduct(
		...
		return ResponseEntity
			.ok(ResponseDto.of(HttpStatus.OK, "제품 수정 성공", dto));
	}
	...
	public ResponseEntity<ResponseDto<Void>> deleteProduct(
		...
		return ResponseEntity
			.ok(ResponseDto.of(HttpStatus.OK, "제품 삭제 성공", null));
	}
}
